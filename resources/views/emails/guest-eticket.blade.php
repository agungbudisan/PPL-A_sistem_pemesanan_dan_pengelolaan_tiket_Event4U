...
<head>
    <meta charset="utf-8">
    <title>E-Ticket untuk Tamu</title>
    <style>
        body {
...
<body>
    <div class="header">
        <h1>E-Ticket untuk Tamu</h1>
    </div>

    <p>Halo {{ $order->guest_name ?? $order->name ?? 'Pelanggan' }},</p>

    <p>Terima kasih atas pembelian tiket Anda sebagai tamu. Berikut adalah e-ticket untuk acara <strong>{{ $order->ticket->event->title }}</strong>.</p>

    <div class="guest-note">
        <p><strong>Catatan Tamu:</strong> Anda membeli tiket ini sebagai tamu. Untuk pengalaman yang lebih baik di masa depan, kami sarankan untuk membuat akun di platform kami.</p>
    </div>
    ...
    <div class="ticket">
        ...
        <div class="details">
            <table>
                ...
                <tr>
                    <td>Nama:</td>
                    <td>{{ $order->guest_name ?? $order->name ?? 'Pelanggan' }}</td>
                </tr>
                <tr>
                    <td>Email:</td>
                    <td>{{ $order->email }}</td>
        ...
        <div class="qrcode" style="text-align: center; margin: 20px 0;">
            ...
            @if(isset($hasQrCode) && $hasQrCode)
                <p style="background-color: #f8f9fa; padding: 10px; border-radius: 5px; border-left: 4px solid #4a86e8;">
                    <strong>QR Code Tiket</strong> telah dilampirkan ke email ini sebagai file PNG.
                    Harap simpan dan tunjukkan saat masuk ke acara.
                </p>
            @else
